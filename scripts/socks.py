...
for name in ("sendto", "send", "recvfrom", "recv"):
    ...
    # Determine if the method is not defined the usual way as a function in the class.
    # Python 2 uses __slots__, so there are descriptors for each method, but they are not functions.
...
class socksocket(_BaseSocket):
    ...
    def set_proxy(self, proxy_type=None, addr=None, port=None, rdns=True, username=None, password=None):
        ...
        proxy_type -  The type of the proxy to be used. Three types are supported: PROXY_TYPE_SOCKS4 (including socks4a), PROXY_TYPE_SOCKS5 and PROXY_TYPE_HTTP
        addr -        The address of the server (IP or DNS).
        port -        The port of the server. Defaults to 1080 for SOCKS servers and 8080 for HTTP proxy servers.
        rdns -        Should DNS queries be performed on the remote side (rather than the local side). The default is True. Note: This has no effect with SOCKS4 servers.
        username -    Username to authenticate with to the server. The default is no authentication.
        password -    Password to authenticate with to the server. Only relevant when username is also provided."""
    ...
    def bind(self, *pos, **kw):
        """Implements proxy connection for UDP sockets. Happens during the bind() phase."""
        ...
        # Need to specify actual local port because some relays drop packets if a port of zero is specified.
    ...
    def send(self, bytes, flags=0, **kwargs):
        log.debug('send, [%s]', bytes.tobytes())  # memoryview
    ...
    def recvfrom(self, bufsize, flags=0):
        log.debug('recvfrom, bufsize: %s', bufsize)
        if self.type != socket.SOCK_DGRAM:
            log.debug('not udp, call super::recvfrom')
        ...
        received_buf = buf.read(bufsize)
        log.debug('recvfrom, %s:%s, [%s]', fromhost, fromport, received_buf)
        return (received_buf, (fromhost, fromport))

    def recv(self, *pos, **kw):
        log.debug('recv')
        bytes, _ = self.recvfrom(*pos, **kw)
        log.debug('recv, len: %s, %s', len(bytes), bytes[:20] + b'...')
        return bytes
    ...
    def _SOCKS5_request(self, conn, cmd, dst):
        ...
        try:
            ...
            if username and password:
                # The username/password details were supplied to the set_proxy method so we support the USERNAME/PASSWORD authentication (in addition to the standard none).
                writer.write(b"\x05\x02\x00\x02")
            else:
                # No username/password were entered, therefore we only support connections with no authentication.
                writer.write(b"\x05\x01\x00")

            # We'll receive the server's response to determine which method was selected
            writer.flush()
            chosen_auth = self._readall(reader, 2)

            if chosen_auth[0:1] != b"\x05":
                # Note: string[i:i+1] is used because indexing of a bytestring via bytestring[i] yields an integer in Python 3
                raise GeneralProxyError("SOCKS5 proxy server sent invalid data")

            # Check the chosen authentication method

            if chosen_auth[1:2] == b"\x02":
                # Okay, we need to perform a basic username/password authentication.
                if not (username and password):
                    # Although we said we don't support authentication, the server may still request basic username/password authentication
                ...
                if auth_status[0:1] != b"\x01": # Bad response
                    raise GeneralProxyError("SOCKS5 proxy server sent invalid data")

                if auth_status[1:2] != b"\x00": # Authentication failed
    ...
    def _write_SOCKS5_address(self, addr, file):
        ...
        log.debug('write_socks5_address, %s', addr)
        host, port = addr
        proxy_type, _, _, rdns, username, password = self.proxy
        family_to_byte = {socket.AF_INET: b"\x01", socket.AF_INET6: b"\x04"}

        # If the given destination address is an IP address, we'll use the IP address request even if remote resolving was specified.
        ...
        if rdns:
            ...
            # We can't really work out what IP is reachable, so just pick the first.
    ...
    def connect(self, dest_pair, catch_errors=None):
        ...
        dest_pair - 2-tuple of (IP/hostname, port).
        """
        log.debug('connect, dest: %s', dest_pair)
        if len(dest_pair) != 2 or dest_pair[0].startswith("["):
            # Probably IPv6, not supported -- raise an error, and hope Happy Eyeballs (RFC6555) makes sure at least the IPv4 connection works...
        ...
        # We set the timeout here so that we don't hang in connection or during negotiation.
        super(socksocket, self).settimeout(self._timeout)
        ...
        try:
            super(socksocket, self).connect(proxy_addr) # Initial connection to proxy server.
    ...
    def connect_ex(self, dest_pair):
        ...
        Like connect(address), but return an error indicator instead of raising an exception for errors returned by the C-level
        connect() call (other problems, such as "host not found" can still raise exceptions).
        """
        log.debug('connect_ex')
        try:
