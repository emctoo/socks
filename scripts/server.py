...
def make_logger(log_path=None, log_level_str='DEBUG'):
...
class Socks5Server:
    def __init__(self, host, port, logger, backlog=128):
        ...
        self.client_dest_map = {}  # This holds client_sock -> dest_sock and dest_sock -> client_sock mappings
        self.sock_send_buffers = {}  # Maps from sock -> buffer to send to sock
    ...
    def establish_socks5(self, sock):
        ...
        self.logger.debug('establish_socks5')
        dest_host, dest_port = None, None
        try:
            self.logger.debug('establish_socks5 - negotiate')
            ver, nmethods, methods = sock.recv(1), sock.recv(1), sock.recv(1)
            sock.sendall(VER + METHOD)  # 不鉴权

            self.logger.debug('establish_socks5 - request')
            ver, cmd, rsv, address_type = sock.recv(1), sock.recv(1), sock.recv(1), sock.recv(1)
            dst_addr = None
            dst_port = None  # bytes, 0x0000
            ...
            if cmd == CMD_TYPE_TCP_BIND:
                self.logger.error('tcp bind request is not supported')
                sock.close()
            elif cmd == CMD_TYPE_UDP:
                self.logger.error('udp request is not supported')
                sock.close()
            elif cmd == CMD_TYPE_CONNECT:
                sock.sendall(VER + SUCCESS + b'\x00' + b'\x01' + encode_str(server_ip + chr(self.port // 256) + chr(self.port % 256)))
                dest_host, dest_port = dst_addr, dst_port
            else:
                # unsupport/unknown command
                self.logger.error('unsupported/unknown request: %s', cmd)
                sock.sendall(VER + UNSUPPORTED_CMD + encode_str(server_ip + chr(self.port // 256) + chr(self.port % 256)))
                sock.close()
    ...
    def handle_connect_thread(self, client_sock, addr):
        ...
        try:
            ...
            self.logger.error('Failed to connect to requested destination (%s:%d) due to error: %s' % (dest_host, dest_port, e))
            client_sock.close()
            return None
        ...
        self.logger.info('SOCKS5 proxy from %s:%d to %s:%d established' % (addr[0], addr[1], dest_host, dest_port))

    def accept_connection(self):
        (client, addr) = self.server_sock.accept()  # socket, (remote_host, remote_port)
        t = threading.Thread(target=self.handle_connect_thread, args=(client, addr))
        t.daemon = True
        t.start()
        self.logger.info('thread created for %s:%s', addr[0], addr[1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--host', action='store', default='0.0.0.0', help='IP/Hostname to serve on', type=str)
    parser.add_argument('-p', '--port', action='store', default=2080, help='Port to serve on', type=int)
    parser.add_argument('--log-path', action='store', default=None, help='DEBUG, INFO, WARNING, ERROR, or CRITICAL', type=str)
    parser.add_argument('--log-level', action='store', default='INFO', help='Log file path', type=str)
